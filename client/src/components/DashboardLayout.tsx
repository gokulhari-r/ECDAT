...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <SidebarProvider defaultOpen>
    <Sidebar collapsible="icon" className="sih-demo-sidebar border-r text-slate-100">
      <SidebarHeader className="sih-demo-sidebar-header">
        <button onClick={() => navigate("/")} className="sih-demo-brand" aria-label="Go to ECDAT Command Center">
          ...
          <span className="group-data-[collapsible=icon]:hidden"><span className="sih-demo-brand-name">ECDAT</span><span className="sih-demo-brand-subtitle">SIH 2026 // PS 26164</span></span>
        </button>
      </SidebarHeader>
      <SidebarContent className="sih-demo-sidebar-content">
        <div className="sih-demo-mission group-data-[collapsible=icon]:hidden"><span className="sih-demo-eyebrow">Judge demo build</span><strong>Crypto assurance<br />mission control</strong><span>DISCOVER · ASSESS · MIGRATE</span></div>
        ...
        <SidebarMenu><SidebarMenuItem><SidebarMenuButton onClick={openDemoLab} tooltip="Open Remediation Lab in a new tab" className="sih-demo-lab-link"><FlaskConical className="h-4 w-4 shrink-0" /><span>Remediation Lab</span><ExternalLink className="ml-auto h-3 w-3 opacity-70 group-data-[collapsible=icon]:hidden" /></SidebarMenuButton></SidebarMenuItem></SidebarMenu>
      </SidebarContent>
    ...
    {initializing ? <div aria-hidden="true" className="sih-demo-boot pointer-events-none fixed inset-0 z-[80] grid place-items-center"><div className="flex flex-col items-center"><span className="sih-demo-boot-mark"><Compass className="h-7 w-7" /></span><p className="mt-5 font-display text-xl font-bold tracking-[.18em] text-white">ECDAT</p><p className="mt-2 text-[10px] font-bold uppercase tracking-[.26em] text-[#fdc448]">SIH competition demonstrator</p><span className="sih-demo-boot-line"><span /></span><p className="mt-3 text-[11px] text-slate-500">Establishing the evidence command deck</p></div></div> : null}
  </SidebarProvider>;
}
