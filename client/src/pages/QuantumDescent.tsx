...
export default function QuantumDescent() {
  ...
  const enterSpatialMode = () => {
    ...
    setLocation("/descent?spatial=1");
  };

  const enterHighestRiskSpatialMode = () => {
    const focus = highestRiskFinding(findings);
    if (!focus) return;
    setSelectedFindingKey(focus.findingKey);
  ...
  const detail = normalLevel === 0 ? `${quantumReadiness}% quantum readiness across the active scan.` : normalLevel === 1 ? `${displayName} is the active service context with ${relationships.length} observed relationships.` : normalLevel === 2 ? `${findings.length} sampled cryptographic findings with provenance.` : `${hndlCount} potential HNDL indicators and ${recommendations.length} prioritised action paths.`;
  const stageCounts = [`${totalAssets} assets`, `${relationships.length} links`, `${findings.length} findings`, `${recommendations.length} actions`];
  return <div className="mx-auto max-w-[1350px]"><Breadcrumb section="Quantum Descent" /><EcdatHeader eyebrow="Signature navigation mode" title="Descend from posture to proof." description="Explore the normal hierarchy or enter a data-backed Spatial Mode to trace observed cryptographic assets, dependencies, risk context, and generated migration paths." /><div className="mt-6 flex flex-col gap-3 rounded-2xl border border-cyan-200/15 bg-cyan-300/[0.055] p-4 md:flex-row md:items-center md:justify-between"><div><p className="text-sm font-medium text-cyan-100">Spatial Mode is ready for the active scan.</p><p className="mt-1 text-xs leading-5 text-slate-400">Interactive clusters and links are derived from the same scan evidence as Inventory, Dependency Intelligence, PQC Guidance, and the Migration Roadmap.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={enterHighestRiskSpatialMode} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-rose-200/25 bg-rose-300/[0.10] px-4 py-2.5 text-xs font-semibold text-rose-100 hover:bg-rose-300/[0.16]"><Focus className="h-3.5 w-3.5" />FOCUS HIGHEST RISK</button><button type="button" onClick={enterSpatialMode} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-cyan-200/25 bg-cyan-300/15 px-4 py-2.5 text-xs font-semibold tracking-[0.12em] text-cyan-50 hover:bg-cyan-300/20"><Sparkles className="h-3.5 w-3.5" />ENTER SPATIAL MODE</button></div></div><div className="mt-5 grid gap-5 lg:grid-cols-[0.65fr_1fr]"><section className="rounded-3xl border border-white/8 bg-[#091423] p-5"><div className="flex items-center justify-between"><p className="text-xs leading-5 text-slate-500">Normal-mode navigation levels</p><span className="text-[10px] font-semibold uppercase tracking-[0.15em] text-cyan-200/60">Depth {normalLevel + 1}/4</span></div><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-white/5"><div className="h-full rounded-full bg-gradient-to-r from-cyan-300 to-emerald-300 transition-all" style={{ width: `${((normalLevel + 1) / normalStages.length) * 100}%` }} /></div><div className="mt-5 space-y-2">{normalStages.map((stage, index) => { const Icon = stage.icon; return <button key={stage.level} type="button" onClick={() => setNormalLevel(index)} aria-current={normalLevel === index ? "step" : undefined} className={`flex w-full items-center gap-3 rounded-2xl p-3 text-left transition ${normalLevel === index ? "bg-cyan-300/10 text-cyan-100" : "text-slate-500 hover:bg-white/[0.035] hover:text-slate-300"}`}><span className="grid h-9 w-9 place-items-center rounded-xl bg-white/5"><Icon className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block text-[10px] font-semibold uppercase tracking-[0.15em] opacity-60">{stage.level} · {stageCounts[index]}</span><span className="block text-sm font-medium">{stage.title}</span></span>{normalLevel === index ? <ChevronRight className="h-3.5 w-3.5" /> : null}</button>; })}</div></section><section className="relative min-h-[430px] overflow-hidden rounded-3xl border border-cyan-200/12 bg-[radial-gradient(circle_at_center,rgba(34,211,238,.14),transparent_18%),radial-gradient(circle_at_center,rgba(14,116,144,.13),transparent_43%),#071423] p-6"><div className="absolute inset-0 grid place-items-center"><div className="h-[76%] w-[76%] rounded-full border border-cyan-200/10" /><div className="absolute h-[52%] w-[52%] rounded-full border border-cyan-200/14" /><div className="absolute h-[30%] w-[30%] rounded-full border border-cyan-200/20" /></div><div className="relative z-10 flex h-full flex-col items-center justify-center text-center"><span className="grid h-16 w-16 place-items-center rounded-3xl border border-cyan-200/25 bg-cyan-300/10 text-cyan-100 shadow-[0_0_50px_rgba(34,211,238,.15)]"><StageIcon className="h-7 w-7" /></span><p className="mt-6 text-[11px] font-semibold uppercase tracking-[0.2em] text-cyan-200/60">{activeStage.level} / {activeStage.title}</p><h2 className="mt-2 font-display text-3xl font-semibold tracking-[-0.04em] text-white">{activeStage.text}</h2><p className="mt-3 max-w-md text-sm leading-6 text-slate-400">{detail}</p>{normalLevel < normalStages.length - 1 ? <button type="button" onClick={() => setNormalLevel(normalLevel + 1)} className="mt-7 inline-flex items-center gap-2 rounded-xl border border-cyan-200/20 bg-cyan-300/10 px-4 py-2 text-xs font-medium text-cyan-100 hover:bg-cyan-300/15">Descend one level <ChevronRight className="h-3.5 w-3.5" /></button> : <button type="button" onClick={enterHighestRiskSpatialMode} className="mt-7 inline-flex items-center gap-2 rounded-xl border border-rose-200/20 bg-rose-300/[0.08] px-4 py-2 text-xs font-medium text-rose-100 hover:bg-rose-300/[0.14]">Investigate highest risk <Focus className="h-3.5 w-3.5" /></button>}</div></section></div></div>;
}
