  ...
  if (isSpatialMode) {
    return <div className="spatial-mode-shell">
      ...
      <main className="spatial-mode-main">
        <aside className="spatial-mode-rail"><p className="px-1 text-[10px] font-semibold uppercase tracking-[0.17em] text-slate-600">Explore</p><div className="mt-2 space-y-1">{spatialLenses.map(item => { const Icon = item.icon; return <button type="button" key={item.id} onClick={() => { setLens(item.id); if (item.id === "risk") focusHighestRisk(); if (item.id === "dependency" && selectedFinding) setView("artefact"); }} className={`flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs ${lens === item.id ? "bg-cyan-300/[0.11] text-cyan-100" : "text-slate-500 hover:bg-white/[0.035] hover:text-slate-200"}`}><Icon className="h-3.5 w-3.5" />{item.label}</button>; })}</div><div className="mt-5 border-t border-white/[0.07] pt-4"><button type="button" onClick={focusHighestRisk} className="flex w-full items-center gap-2 rounded-xl border border-rose-200/15 bg-rose-300/[0.055] px-3 py-2 text-left text-xs font-medium text-rose-100 hover:bg-rose-300/[0.1]"><Focus className="h-3.5 w-3.5" />Focus highest risk</button><button type="button" onClick={() => { setView("enterprise"); setLens("enterprise"); setSelectedClusterId(undefined); setSelectedSceneId(undefined); setCameraResetToken(token => token + 1); }} className="mt-2 flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs text-slate-500 hover:bg-white/[0.035] hover:text-slate-200"><Orbit className="h-3.5 w-3.5" />Reset view</button></div><div className="mt-auto rounded-xl border border-white/[0.07] bg-white/[0.025] p-3"><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-600">Active scan</p><p className="mt-1 text-sm font-medium text-slate-200">{totalAssets} assets</p><p className="mt-1 text-[11px] leading-4 text-slate-500">{criticalCount} critical · {quantumVulnerableCount} quantum-vulnerable · {quantumReadiness}% readiness</p></div></aside>
        <section className="spatial-stage"><div className="absolute left-5 top-5 z-10 rounded-2xl border border-white/10 bg-[#08111f]/85 p-3"><p className="text-[10px] font-semibold uppercase tracking-[0.17em] text-cyan-200/60">{lens} lens / {view} level</p><p className="mt-1 max-w-[260px] text-xs leading-5 text-slate-400">{view === "enterprise" ? "Clusters are calculated from the active scan and limit detail until you descend." : "Entities and paths represent observed scan relationships at the selected level."}</p></div><div className="absolute bottom-4 left-5 z-10 flex gap-2">{webglCapability === "webgl" ? <><span className="rounded-full border border-white/10 bg-[#08111f]/85 px-2.5 py-1 text-[10px] text-slate-400">Drag to free look</span><span className="rounded-full border border-white/10 bg-[#08111f]/85 px-2.5 py-1 text-[10px] text-slate-400">WASD to move</span><span className="rounded-full border border-white/10 bg-[#08111f]/85 px-2.5 py-1 text-[10px] text-slate-400">R / F height</span></> : <span className="rounded-full border border-white/10 bg-[#08111f]/85 px-2.5 py-1 text-[10px] text-slate-400">Select an observed entity to descend</span>}</div>{webglCapability === "webgl" ? <SpatialScene clusters={clusters} graphNodes={sceneGraphNodes} edges={sceneEdges} view={view} resetKey={`${cameraResetToken}:${view}:${selectedClusterId ?? ""}:${selectedSceneId ?? ""}:${selectedFindingKey ?? ""}`} selectedId={selectedSceneId ?? selectedFindingNode} onSelect={selectSceneEntity} /> : <SpatialFallback entities={fallbackEntities} selectedId={selectedSceneId ?? selectedFindingNode} onSelect={selectSceneEntity} />}</section>
