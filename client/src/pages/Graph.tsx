  ...
  return <div className="mx-auto max-w-[1620px]">
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
      <section className="overflow-hidden rounded-3xl border border-white/8 bg-[radial-gradient(circle_at_48%_0%,rgba(34,211,238,0.1),transparent_38%),#091423]">
        ...
        <svg ref={svgRef} className="h-[590px] min-w-[880px] w-full touch-none select-none cursor-grab active:cursor-grabbing" viewBox="0 0 1120 620" role="img" aria-label="Interactive graph of observed cryptographic relationships. Drag or two-finger scroll to pan. Pinch on a trackpad to zoom." onPointerDown={event => { dragOrigin.current = { x: event.clientX, y: event.clientY, panX: transform.x, panY: transform.y, moved: false }; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={event => { const origin = dragOrigin.current; if (!origin) return; const rect = event.currentTarget.getBoundingClientRect(); const dx = (event.clientX - origin.x) * (1120 / rect.width); const dy = (event.clientY - origin.y) * (620 / rect.height); if (Math.abs(dx) + Math.abs(dy) > 3) origin.moved = true; setTransform(current => panGraphViewport({ ...current, x: origin.panX, y: origin.panY }, dx, dy)); }} onPointerUp={event => { if (!dragOrigin.current) return; dragOrigin.current = null; event.currentTarget.releasePointerCapture(event.pointerId); }} onPointerCancel={() => { dragOrigin.current = null; }} onWheel={event => { event.preventDefault(); const rect = event.currentTarget.getBoundingClientRect(); const pointX = (event.clientX - rect.left) * (1120 / rect.width); const pointY = (event.clientY - rect.top) * (620 / rect.height); if (event.ctrlKey) { const zoomFactor = Math.exp(-event.deltaY * 0.004); setTransform(current => zoomGraphAtPoint(current, current.scale * zoomFactor, pointX, pointY)); return; } const deltaX = (event.deltaX || (event.shiftKey ? event.deltaY : 0)) * (1120 / rect.width); const deltaY = (event.shiftKey ? 0 : event.deltaY) * (620 / rect.height); setTransform(current => panGraphViewport(current, -deltaX, -deltaY)); }}>
        ...
        <div className="flex flex-wrap items-center gap-3 border-t border-white/8 bg-[#06101c]/50 px-5 py-3 text-[11px] text-slate-500"><Move className="h-3.5 w-3.5" />Drag or two-finger scroll to pan · pinch on a trackpad or use controls to zoom · Escape clears selection</div>
      </section>
