...
## CodeHunters Academy archive-derived UI replacement

The user-supplied `codehuntersacademy-design.zip` was extracted and inspected as the visual source of truth. Its documented black canvas (`#000000`), charcoal surface (`#191c1f`), divider (`#212529`), active pink (`#f9004d`), warm warning/danger colors, 5-pixel spacing cadence, Rajdhani display type, and Rubik body type replaced the prior ECDAT observatory layer. The archive reference mapping is retained in `codehunters-design-reference.md`.

ECDAT now uses a compact CodeHunters-style editorial header with an ECDAT mark, horizontally scrollable workspace navigation, search, theme, notifications, and protected-account controls. The former fixed sidebar and orbital/grid visual framing are no longer rendered. Shared headers, panels, forms, cards, states, chart framing, graph canvas, migration posture panel, and standalone Lab now use the charcoal-card and warm-pink design language. All existing route, scan, graph button-zoom, evidence, migration, report-export, and Lab behaviors remain intact.

Command Center, CBOM Inventory, Dependency Graph, Migration, Evidence & Reports, and Remediation Lab were reviewed at desktop size. Command Center, Inventory, and Lab were additionally checked at 390 pixels; the top navigation remains horizontally reachable and workspace content reflows without page-level clipping. The maintained alternate light theme includes neutral readable surfaces and pink focus/active states. TypeScript passed and Vitest completed **66 tests across twenty-three files**.
